...
/**
 * Implementación del servicio de IA usando DeepSeek API.
 * Modelo: deepseek-chat (DeepSeek-V3, excelente relación coste/calidad).
 *
...
public class DeepSeekService implements AIService {
    ...
    public String getCropRecommendation(Crop crop, List<SensorReading> readings) {
        if (!enabled) return "[DeepSeek desactivado] Configure su API key.";
    ...
    @Override
    public String getProviderName() { return "DeepSeek (V3)"; }
    ...
    // DeepSeek es compatible con el formato OpenAI
        ...
        try {
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    return "[Error DeepSeek] Código: " + response.code() +
                           " - " + (response.body() != null ? response.body().string() : "Sin respuesta");
                }
            }
        } catch (IOException e) {
            return "[Error DeepSeek] No se pudo conectar: " + e.getMessage();
        }
    }
}
